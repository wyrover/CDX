...
void CDXSurface::TextureMap(CDXSurface *dest, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, RECT* area, BOOL bTrans)
{
    ...
    if (y2 < y1) {
        Scanleftside(x2, x1, y2, y1 - y2, 1, TexWidth, TexHeight, dClipRect);
    } else        {
        Scanrightside(x1, x2, y1, y2 - y1, 1, TexWidth, TexHeight, dClipRect);
    }

    if (y3 < y2) {
        Scanleftside(x3, x2, y3, y2 - y3, 2, TexWidth, TexHeight, dClipRect);
    } else        {
        Scanrightside(x2, x3, y2, y3 - y2, 2, TexWidth, TexHeight, dClipRect);
    }

    if (y4 < y3) {
        Scanleftside(x4, x3, y4, y3 - y4, 3, TexWidth, TexHeight, dClipRect);
    } else        {
        Scanrightside(x3, x4, y3, y4 - y3, 3, TexWidth, TexHeight, dClipRect);
    }

    if (y1 < y4) {
        Scanleftside(x1, x4, y1, y4 - y1, 4, TexWidth, TexHeight, dClipRect);
    } else        {
        Scanrightside(x4, x1, y4, y1 - y4, 4, TexWidth, TexHeight, dClipRect);
    }
    ...
    if (m_PixelFormat.bpp == 8) {
        if (bTrans) {
            for (y = miny; y <= maxy; y++) {
                ...
                if (polyx2 > dClipRect->right - 0) {
                    polyx2 = dClipRect->right - 0;
                }
